...
public class TokenStateKeeperTests
{
    ...
    public void ParallelStage_1000Tokens_DoesNotThrow()
    {
        var sut = Create();
        const int n = 1000;
        for (var i = 0; i < n; i++) sut.Seed(i, $"V{i}");
        Parallel.For(0, n, i => sut.Stage(i, i + n, $"V{i}*"));
        sut.Commit();
    ...
    public void Stress_Commit100k_UnderTwoSeconds()
    {
        ...
        for (var i = 0; i < count; i++) sut.Seed(i, $"V{i}");
        for (var i = 0; i < count; i++) sut.Stage(i, i + count, $"V{i}*");
    ...
    public void Concurrent_StageAndCommit_MultiThreadIntegrity()
    {
        ...
        Parallel.For(0, threads, _ =>
        {
            ...
            for (var i = 0; i < perThread; i++) sut.Seed(start + i, "I");
        });

        Parallel.For(0, threads, t =>
        {
            var offset = t * perThread;
            for (var i = 0; i < perThread; i++) sut.Stage(offset + i, offset + i + 50_000, "M");
        });
